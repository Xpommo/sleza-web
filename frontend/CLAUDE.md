...
- **Current flow (`/app/register` → `/app/sites` → `/app/start/*` → `/app/site/*`)** — state lives in `sessionStorage` via `app/app/start/_shared/anketaState.js`. Billing lives at `/app/billing` in the account menu next to «Мои сайты» (owner decision 18.09), but since 23.09 **each site has its own tariff, subscription year and renewal date**; only the payment method, card, payer and acts e-mail are shared. Paying, changing tariff and switching a site off happen only in that site's row in «Подписка» — «Обзор» just shows the state and links there. Subscription state is derived only in `app/app/site/_shared/subscription.js` — overview, «Подписка» and the «Мои сайты» card must not derive it separately.
