...
export function ownerLabels(owner) {
  ...
  return {
    ...
    name: isOoo ? 'Наименование' : 'ФИО',
    namePlaceholder: isOoo ? 'ООО «Ромашка»' : 'Иванова Мария Сергеевна',
    // у самозанятого регистрационного номера нет
    ogrn: owner === 'Самозанятый' ? null : isOoo ? 'ОГРН' : 'ОГРНИП',
    ogrnLength: isOoo ? 13 : 15,
    address: isOoo ? 'Юридический адрес' : 'Адрес для почтовой связи',
    addressPlaceholder: isOoo ? 'Индекс, город, улица, дом' : 'Можно абонентский ящик',
  };
}
...
export function validateRequisites(v) {
  ...
  if (!String(v.companyMail || '').trim()) e.companyMail = 'Нужна почта вида name@site.ru — её увидят в реквизитах на сайте.';
