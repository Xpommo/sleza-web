...
// (у каждого сайта свой тариф и год подписки, отключение в «Моих сайтах»,
// реквизиты в «Документах»), здесь они приведены к действительности.
const FAQ = [
  [
    'Как оплачивается подписка?',
    'С баланса аккаунта: пополните его в «Балансе и платежах» картой или по счёту, а оплата года каждого сайта спишется сама в его дату продления. Оплатить можно и заранее, даже за несколько лет: в «Моих сайтах», вид «Таблица», в строке сайта. Если на балансе не хватит, доплатите там же. Срок продлится на 12 месяцев.',
  ],
  [
    'Что будет, если на балансе не хватит денег?',
    'Год сайта не продлится, но не сразу: сайт проработает ещё 3 дня, а если выставлен счёт, то пока ждём оплату. Потом виджет отключится, а обновление документов по закону остановится. Напомним письмом заранее. Битой ссылки на сайте не появится: уже опубликованные страницы документов остаются доступными.',
  ],
  [
    'Как обновить реквизиты компании?',
    'В разделе «Документы» сайта, в блоке «Что можно изменить», нажмите «Изменить» у «Реквизитов владельца» и поправьте поля сами. Документ и подвал сайта обновятся вместе с ними. Реквизиты для счёта отдельные, они в «Балансе и платежах».',
  ],
  ['Могу я добавить второй сайт?', 'Да, в «Моих сайтах» нажмите «Добавить сайт». Документы, виджет, тариф и год подписки у каждого сайта свои: оплата его года спишется с того же баланса, в его дату.'],
  [
    'Как отключить сайт?',
    'В «Моих сайтах», вид «Таблица», в строке сайта: «⋯» → «Отключить сайт». Сайт проработает до конца оплаченного срока, дальше продлевать его не будем; остальные сайты это не затронет. Если не хотите автоматических списаний, отключать сайт не нужно: выключите «Автопродление» и продлевайте вручную.',
  ],
  // Акты вынесены в свой раздел (владелец 24.09) — бухгалтер их ищет.
  ['Где чеки, счета и акты?', 'В «Балансе и платежах», на вкладке «Акты и чеки»: e-mail, на который они приходят, и акты за оплаченные годы сайтов. Историю операций по балансу смотрите на вкладке «Платежи».'],
  [
...
export default function SupportClient() {
  ...
  function send() {
    ...
    setSent(t.no);
    setText('');
    announce(`Обращение № ${t.no} отправлено. Ответ придёт сюда, в «Мои обращения».`);
  }
            ...
            {sent ? (
              ...
              <>
                ...
                <p className="mt-2 text-[13px] leading-5 text-ink/60">
                  Ответ придёт сюда же, в «Мои обращения». На e-mail пришлём только уведомление, что вам ответили.
                </p>
                <button
                  type="button"
                  onClick={send}
