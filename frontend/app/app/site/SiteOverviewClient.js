...
export default function SiteOverviewClient() {
  ...
  if (b.cancelled && state === 'paid') {
    ...
    const enough = balanceOf(acc) >= PRICE;
    const card = acc.billing?.card?.auto ? acc.billing.card : null;
    banner = {
      tone: lastDay ? 'warn' : 'info', Icon: ClockIcon, title: `Пробный период — до ${trialEnds(a)}`,
      text: a.installed
        ? <>Документы и виджет уже работают на {domain}. {b.cancelled ? 'Автопродление выключено — после этой даты сайт отключится.' : enough ? 'Когда пробный период закончится, спишем оплату года с баланса — на нём хватает.' : card ? `Когда пробный период закончится, спишем оплату года с баланса, а недостающее — с карты ···· ${card.last4}.` : 'Когда пробный период закончится, спишем оплату года с баланса — пополните его заранее. Напомним письмом за день до конца.'}</>
  ...
  // Тариф выбирают в «Подписке» во время пробного периода (владелец 24.09);
  // пока не выбран — так и пишем, без подставленного «Тарифа Х».
  const afterRow = [
    'Дальше',
    tariff ? `${tariff} · ${PRICE_LABEL} в год` : a.trialStartedAt ? `Выберите тариф до ${trialEnds(a)}` : 'Тариф — на выбор в «Подписке»',
  ];
  const subRows = {
