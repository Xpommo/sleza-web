/**
 * TG-бот Фонарик (локальный поллинг).
 *
 * Запуск (из папки backend):
 *   node src/agent/bot.mjs
 *
 * Нужно в backend/.env:
 *   TELEGRAM_BOT_TOKEN=<токен от @BotFather>
 *   TELEGRAM_CHAT_ID=<твой chat id>
 *   DEFAULT_GROQ_KEY=<уже есть>
 */
...
try { process.loadEnvFile(ENV_PATH); } catch { /* env может быть задан снаружи */ }
...
if (!CHAT_ID) console.warn('⚠️  TELEGRAM_CHAT_ID не задан — бот будет отвечать любому.');

const { reply } = await import('./agent.js');
const { llmProvider } = await import('./llm.js');
const { extractHostname, getScanByUuid, formatScanContext } = await import('./scanLookup.js');
const SCANNER_URL = process.env.SCANNER_URL || 'https://fonarik-web.vercel.app';
const { summaryCard, findingCard, fixCard, risksCard, docsCard, doneCard, isProblem, esc } = await import('./funnel.js');
const { loadStore, saveStore } = await import('./store.js');
const logEvent = (type, host) => console.log(`[event] ${type}${host ? ' · ' + host : ''}`);
...
async function onMessage(msg) {
  ...
  // Специалист набирает ответ (после кнопки «Ответить»)
...
async function onCallback(cb) {
  ...
  // Специалист выбирает, кому ответить
  ...
  if (action === 'nm') {
    ...
    await sendCard(chatId, {
      ...
      keyboard: kb([[{ text: '📞 Да, специалисту', callback_data: 'f:human' }], [{ text: '➡️ Дальше', callback_data: `f:walk:${idx + 1}` }]]),
    });
    return;
  }
}
  ...
  try {
    ...
    if (r?.result?.length) {
      for (const u of r.result) {
        ...
        if (u.message) await onMessage(u.message).catch(e => console.error('onMessage:', e.message));
        else if (u.callback_query) await onCallback(u.callback_query).catch(e => console.error('onCallback:', e.message));
      }
    }
  } catch (e) {
    console.error('[poll] ошибка:', e.message);
  }
}
